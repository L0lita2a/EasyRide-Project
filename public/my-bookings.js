...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchHistory() {
        try {
            const response = await fetch(`/api/user/bookings?user_id=${encodeURIComponent(currentUser.id)}`);
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Failed to load bookings.');
            renderHistory(data.bookings || []);
        } catch (error) {
            console.warn('Using demo bookings because API is unavailable:', error.message);
            renderHistory(typeof DEMO_BOOKINGS !== 'undefined' ? DEMO_BOOKINGS : []);
        }
    }
    ...
    window.cancelBooking = async (id, btnEl) => {
        ...
        try {
            const response = await fetch('/api/cancel-booking', {
                ...
                body: JSON.stringify({ bookingId: id, user_id: currentUser.id })
            });
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Cancellation failed.');
        } catch (error) {
            showToast(error.message || 'Could not cancel booking.');
            return;
        }

        const card = btnEl.closest('.history-card');
        const detailsDiv = card.querySelector('.history-details');
        if (detailsDiv) {
