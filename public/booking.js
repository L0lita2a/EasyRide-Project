document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchCar() {
        try {
            ...
            if (!response.ok) throw new Error(data.error || 'Car not found.');
            selectedCar = data;
        } catch (error) {
            console.warn('Using demo car because API is unavailable:', error.message);
            selectedCar = (typeof DEMO_CARS !== 'undefined' ? DEMO_CARS : []).find(c => c.id === parseInt(carId)) || null;
        }
    ...
    bookingForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (idEl) idEl.textContent = 'Booking Reference: #ER-' + String(data.booking.id).padStart(4, '0');
